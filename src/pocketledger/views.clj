         ...
         [:style
          (h/raw
           "html, body { margin: 0; padding: 0; height: 100dvh; overflow: hidden; }
            .app-container { height: 100dvh; display: flex; flex-direction: column; padding-top: env(safe-area-inset-top); }
...
")]]
        ...
        [:nav.ty-elevated.border-b.ty-border
         [:div.mx-auto.max-w-3xl.px-4.py-4.flex.items-center.justify-between
          [:div.flex.items-center.gap-2
           [:ty-icon {:name "wallet" :size "md"}]
           [:h1.text-xl.font-bold.ty-text++ "PocketLedger"]
           [:a {:href "/test" :class "text-xs underline ty-text-"} "dbg"]]
          [:div.flex.items-center.gap-2
           [:ty-button {:size "sm" :plain true
                        :onclick "document.documentElement.classList.toggle('dark');
                                  localStorage.setItem('theme',
                                    document.documentElement.classList.contains('dark') ? 'dark' : 'light')"}
            [:ty-icon.theme-moon {:name "moon" :size "sm"}]
            [:ty-icon.theme-sun {:name "sun" :size "sm"}]]]]]
        ...
        [:footer.ty-content.border-t.ty-border
         [:div.mx-auto.max-w-3xl.px-4.py-4.ty-text-.text-sm.text-center
          ...
          " — " [:a {:href "/test" :class "underline"} "debug"]]]
  ...
  (let [bg-class (case flavor
    ...
    [:div {:class [bg-class border-class
     ...
     [:div.flex.items-center.justify-center.gap-1.mb-1
      (when icon [:ty-icon {:name icon :size "xs" :class text-class}])
  ...
  (let [settings (db/get-all-settings)
    ...
    [:div.flex.items-center.justify-between.p-3.ty-elevated.rounded-lg
       ...
       [:div.flex.items-center.gap-2.flex-wrap
        (for [{:keys [name icon]} categories]
          [:ty-tag {:size "sm" :flavor "neutral"}
           [:ty-icon {:name (or icon "tag") :size "xs" :slot "start"}]
           name])
     ...
     [:div.flex.items-center.gap-3.shrink-0
      ...
      [:ty-button
       {:size "xs" :flavor "danger" :plain true
        "data-on:click" (str "@post('/api/transactions/delete?id=" id "')")}
       [:ty-icon {:name "trash" :size "xs"}]]]]))
      ...
      [:div.app-body
       ...
       [:ty-tabs#app-tabs {:active "dashboard"}
        ...
        (for [[id label icon] [["dashboard" "Dashboard" "layout-dashboard"]
          ...
          [:span {:slot (str "label-" id) :class "flex items-center gap-1"}
          ...
          [:div.ty-elevated.rounded-xl.p-6.space-y-4
           ...
           [:div.flex.gap-2
            [:ty-tag
             ...
             [:ty-icon {:name "trending-down" :size "xs" :slot "start"}]
             "Expense"]
            [:ty-tag
             {:size "md"
              "data-attr:flavor" "$txType === 'income' ? 'success' : 'neutral'"
              "data-on:click" "$txType = 'income'"}
             [:ty-icon {:name "trending-up" :size "xs" :slot "start"}]
             "Income"]]
           ...
           [:ty-button
            ...
            [:ty-icon {:slot "start" :name "plus" :size "sm"}]
            "Add Transaction"]
         ...
         [:div.p-4
          ...
          [:div.ty-elevated.rounded-xl.p-6.space-y-4
           ...
           [:div
            ...
            [:ty-button
             {:flavor "danger" :plain true :class "mt-2"
